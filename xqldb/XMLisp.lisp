...
;* Systems      : G4, OS X 10.4.10                                   *
;* Lisps        : MCL 5.0, MCL 5.2, LispWorks 4.3.7, OpenMCL 0.14    *
...
(defmethod READ-XMLISP-ELEMENT-CONTENT ((Self xml-serializer) Stream)
  (let ((Char (read-char Stream)))
    (case Char
      ...
      (t
       (unread-char Char Stream)
       (let ((Element (read-xmlisp-element (read-name-and-make-instance Stream) Stream)))
      ...
      (let ((Char (read-char Stream)))
        (case Char
          ...
          (#\/
           (case (read-char Stream)
             (#\> ;; DONE!
              (return-from read-xmlisp-element (read-return-value Self)))
             (t
              (error "Not a well formed end tag. Missing '>'"))))
          ;; end tag
          (#\<
...
;; error if XMLisp has not been loaded but there allready is a #\< reader
;; there can only be one such reader
(eval-when (:compile-toplevel :load-toplevel :execute)
  (when (and (not (boundp '*Non-XMLISP-Readtable*)) (get-macro-character #\<))
    (warn "~%XMLisp: The current *readtable* allready contains a #/< reader function: ~A" (get-macro-character #\<))))
