...
class AuthRequestHandler:
    """Simple authenticating request 

    It simply overrides handle() and looks for authentication
    headers.

    In order to use it create a subclass of bufAuthRequestHandler
    ...
    # normal handle method but with authorization test
	...
	self.send_header("WWW-Authenticate","Basic realm=\"davsyneredge\"")
...
class bufAuthRequestHandler(BufferingHTTPServer.bufHTTPRequestHandler,AuthRequestHandler):
...
class BasicAuthRequestHandler(BufferingHTTPServer.bufHTTPRequestHandler,AuthRequestHandler):

	def _append(self,s):
		""" write the string to wfile """
		self.wfile.write(s)

class myserver(bufAuthRequestHandler):
	def get_userinfo(self,user):
		if user=="foo":
			return "bar"
		return None

def test(HandlerClass = myserver,
